...
const NOTIFICATIONS: Notification[] = [
  {
    id: 1,
    message: 'Laporan "Jalan berlubang di Jl. Sudirman" berubah ke Dalam Penanganan',
    time: '2 jam lalu',
    read: false,
  },
  {
    id: 2,
    message: 'Laporan "Jembatan pejalan kaki retak" telah Diverifikasi',
    time: '5 jam lalu',
    read: false,
  },
  {
    id: 3,
    message: 'Laporan "Tumpukan sampah TPS" berhasil Selesai',
    time: '1 hari lalu',
    read: true,
  },
]
...
export function WargaDashboard() {
  ...
  const [lightbox, setLightbox] = useState<string | null>(null) // ← state untuk ImageViewer
  ...
  const myStats = [
    ...
    { label: 'Poin', value: MOCK_USER.points, icon: <Star size={15} />, color: '#8B5CF6' },
  ]
  ...
  return (
    <div className="flex flex-col h-full min-h-0 overflow-hidden" style={{ background: '#0F172A' }}>
      {/* Header */}
      <div
        className="px-4 sm:px-6 pt-4 pb-3 border-b border-border flex-shrink-0"
        style={{ background: '#1E293B' }}
      >
        <div className="flex items-center justify-between mb-3 gap-2">
          <div className="min-w-0">
            <h1 className="text-foreground truncate">Hai, {MOCK_USER.name.split(' ')[0]} 👋</h1>
            <p className="text-xs text-muted-foreground mt-0.5 hidden sm:block">
              Pantau dan kelola laporan Anda.
            </p>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="relative">
              <button
                ...
                className="p-2 rounded-lg border border-border transition-colors relative"
              ...
              {showNotif && (
                <>
                  ...
                  <div
                    className="absolute right-0 top-full mt-2 rounded-xl border border-border shadow-2xl z-50 overflow-hidden"
                    style={{
                      background: '#1E293B',
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-lg text-sm font-medium transition-opacity hover:opacity-90"
        ...
        <div className="grid grid-cols-4 gap-2">
          {myStats.map((s) => (
            <div
              key={s.label}
              className="rounded-lg p-2 sm:p-3 border border-border flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3"
              style={{ background: '#0F172A' }}
            >
              <div
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <div
          className={`flex-1 overflow-y-auto p-4 sm:p-6 space-y-3 ${
            activeTab !== 'laporan' ? 'hidden lg:block' : ''
          }`}
        >
          ...
          {reports.map((r) => (
            <div
              ...
              className="rounded-xl p-4 border border-border cursor-pointer transition-all"
              style={{
                background: selected?.id === r.id ? 'rgba(59,130,246,0.06)' : '#1E293B',
              }}
            >
              ...
              {selected?.id === r.id && (
                <div className="mt-4 pt-4 border-t border-border space-y-3">
                  ...
                  <div className="space-y-1.5">
                    {(['dilaporkan', 'diverifikasi', 'dalam_penanganan', 'selesai'] as const).map(
                      (s, i) => {
                        const currentIdx = (
                          ['dilaporkan', 'diverifikasi', 'dalam_penanganan', 'selesai'] as const
                        ).indexOf(r.status)
                        ...
                        return (
                          <div key={s} className="flex items-center gap-2">
                            <div
                              ...
                              style={{
                                background: isReached ? STATUS_CONFIG[s].color : '#334155',
                              }}
                            />
                            <span
                              className={`text-xs ${
        ...
        {/* Sidebar gamifikasi */}
        <div
          className={`overflow-y-auto lg:w-64 lg:border-l lg:border-border lg:flex-shrink-0 ${
            activeTab === 'profil' ? 'flex-1' : 'hidden lg:block'
          }`}
        ...
        >
          <div className="p-4 border-b border-border">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <div className="text-sm text-foreground font-medium">{MOCK_USER.name}</div>
                <div className="text-xs text-muted-foreground">{MOCK_USER.level}</div>
              </div>
            </div>
            <div className="rounded-lg p-3 border border-border" style={{ background: '#0F172A' }}>
              ...
              <div className="h-1.5 rounded-full" style={{ background: '#1E293B' }}>
                <div
                  ...
                  style={{
                    width: `${(MOCK_USER.points % 500) / 5}%`,
                    background: 'linear-gradient(90deg, #F59E0B, #EF4444)',
                  }}
                />
              </div>
              <div className="text-xs text-muted-foreground mt-1.5">
                {500 - (MOCK_USER.points % 500)} poin lagi ke level berikutnya
              </div>
            </div>
          </div>
          ...
          <div className="p-4 border-b border-border">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm text-foreground">Badge Saya</span>
            </div>
          ...
          <div className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm text-foreground">Top Pelapor Minggu Ini</span>
            </div>
            {[
              { name: 'Dewi Purnama', pts: 580, rank: 1 },
              { name: 'Ahmad Fauzi', pts: 420, rank: 2 },
              { name: MOCK_USER.name, pts: MOCK_USER.points, rank: 3 },
            ].map((u) => (
              <div
                key={u.name}
                className="flex items-center gap-2.5 py-2 px-2.5 rounded-lg mb-1"
                style={{
                  background:
                    u.name === MOCK_USER.name ? 'rgba(59,130,246,0.08)' : 'transparent',
                  border:
                    u.name === MOCK_USER.name
              ...
              >
                ...
                <div
                  ...
                  style={{ background: u.rank === 1 ? '#F59E0B' : '#334155' }}
                >
                  {u.name.charAt(0)}
                </div>
