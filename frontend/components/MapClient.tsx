...
import { MOCK_REPORTS, STATUS_CONFIG, CATEGORIES, Report } from '@/components/mockData'
          ...
          >
            <Popup>
              <div style={{ fontSize: 13 }}>
                ...
                <br />
                <span style={{ color: '#666' }}>{CATEGORIES[r.category]?.label}</span>
              </div>
            </Popup>
          </Marker>
        )
    ...
    <>
      ...
      >
        ...
        <div className="p-4 space-y-4">
          ...
          <h3 className="text-sm text-foreground leading-snug">{report.title}</h3>
          ...
          <div className="border-t border-border pt-3">
            <div className="text-xs text-muted-foreground mb-2">Riwayat Status</div>
  ...
  return (
    <div className="flex h-full w-full">
      ...
      <div className="flex-1 relative">
        ...
        >
          <div className="text-muted-foreground mb-1 font-mono text-[10px]">LEGENDA</div>
          {Object.entries(STATUS_CONFIG).map(([key, config]) => (
            <div key={key} className="flex items-center gap-2">
              ...
              />
              <span className="text-foreground text-[10px] sm:text-xs">{config.label}</span>
            </div>
          ))}
        </div>
      </div>
