...
export function ReportForm({ onClose, onSubmit }: ReportFormProps) {
  ...
  const [address, setAddress] = useState('Jl. Jend. Sudirman, Kel. Menteng, Jakarta Pusat')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    // Bisa kirim ke backend di sini (opsional)
    onSubmit({
      title,
      category,
      description,
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)' }}>
        <div className="bg-card border border-border rounded-xl p-10 flex flex-col items-center gap-4 max-w-sm w-full text-center">
          ...
          <p className="text-muted-foreground text-sm">
            Laporan Anda telah diterima dengan ID #URB-2026-009. Kami akan meninjau dan memverifikasi laporan dalam 1×24 jam.
          </p>
          ...
          <button
            onClick={onClose}
            className="mt-2 w-full py-2.5 rounded-lg text-sm font-medium transition-opacity hover:opacity-90"
    ...
    >
      <div className="bg-card border border-border rounded-t-2xl sm:rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          {/* Lokasi */}
          <div
            className="rounded-lg p-3 flex items-start gap-3 border border-border"
            ...
            {imagePreview ? (
              <div className="relative rounded-lg overflow-hidden border border-border">
                ...
                <button
                  ...
                  className="absolute bottom-2 left-2 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-opacity hover:opacity-90 disabled:opacity-60"
              ...
              <label
                className="flex flex-col items-center justify-center h-40 rounded-lg border-2 border-dashed border-border hover:border-primary/50 cursor-pointer transition-colors"
          ...
          <div>
            ...
            <div className="relative">
              <select
                ...
                className="w-full rounded-lg px-3 py-2.5 text-sm text-foreground border border-border outline-none appearance-none pr-10"
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg px-3 py-2.5 text-sm text-foreground border border-border outline-none placeholder:text-muted-foreground"
              style={{ background: 'var(--input-background)' }}
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              required
              rows={3}
              placeholder="Jelaskan kondisi masalah, sejak kapan terjadi, dampak yang dirasakan..."
              className="w-full rounded-lg px-3 py-2.5 text-sm text-foreground border border-border outline-none placeholder:text-muted-foreground resize-none"
              style={{ background: 'var(--input-background)' }}
            />
          </div>
